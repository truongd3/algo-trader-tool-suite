...
///////////////////////////////////////////////////////////
// member funcs
BreakoutTrader::BreakoutTrader(std::string symbol, std::string type, std::string exch, std::string ccy, std::string time, std::string isdbg) : m_pClient(new EPosixClientSocket(this))
	, m_state(ST_CONNECT)
	, m_sleepDeadline(0)
	, m_orderId(0)
	, m_tickId(0)
...
string BreakoutTrader::rollLog() {
	...
	if (tmp != m_curName) {
		...
		if (!*(m_pFout)) {
			...
			*m_pFout << "DAY," << "HOUR," << "MIN," << "SEC," << "TOTAL_BID_SIZE," 
					 << "CLOSE_BID," << "BID_ST_DIV," << "TOTAL_ASK_SIZE," << "CLOSE_ASK,"
					 <<"ASK_ST_DIV," << "CLOSE_PRICE," << "ST_DIV," << endl;
		}
		return tmp;
	}
...
bool BreakoutTrader::connect(const char *host, unsigned int port, int clientId) {
	...
	*m_pLogger << ctime(&now) << "Connecting to: "<< (!( host && *host) ? "127.0.0.1" : host)<<" port: " << port<<" clientId: "<<clientId << endl;
	...
	else *m_pLogger << ctime(&now) << "Cannot connect to: "<< (!( host && *host) ? "127.0.0.1" : host)<<" port: " << port<<" clientId: "<<clientId << endl;
...
void BreakoutTrader::closeTimePeriod() {
	...
	*m_pFout << timeinfo->tm_mday << "," << timeinfo->tm_hour << "," << timeinfo->tm_min << ","
			 << timeinfo->tm_sec << "," << m_totalBidVol << "," << m_lastBidPrice << "," 
			 << scientific << m_BidStDiv << "," << m_totalAskVol << "," << fixed 
			 << m_lastAskPrice << "," << scientific << m_AskStDiv << "," << m_lastPrice 
			 << "," << m_StDiv << endl;
	// clear
	// delete
	m_totalBidVol = 0;
...
void BreakoutTrader::tickSize(TickerId tickerId, TickType field, int size) {
	if (field == BID_SIZE) m_totalBidVol += size;
	else if (field == ASK_SIZE) m_totalAskVol += size;
