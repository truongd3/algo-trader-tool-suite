...
bool EClientSocketBase::DecodeField(IBString& stringValue, const char*& ptr, const char* endPtr) {
	if (!CheckOffset(ptr, endPtr)) return false;
	const char* fieldBeg = ptr;
...
void EClientSocketBase::CleanupBuffer(BytesVec& buffer, int processed) {
	...
	if ((size_t)processed == buffer.size()) {
		if (buffer.capacity() >= BufferSizeHighMark) BytesVec().swap(buffer);
		else buffer.clear();
	} else buffer.erase(buffer.begin(), buffer.begin() + processed);
};
...
///////////////////////////////////////////////////////////
// member funcs
EClientSocketBase::EClientSocketBase(EWrapper *ptr) : m_pEWrapper(ptr), m_clientId(-1), m_connected(false), m_serverVersion(0) {}
...
void EClientSocketBase::reqHistoricalData(TickerId tickerId, const Contract &contract,
	...
	if (Compare(contract.secType, "BAG") == 0) {
		if (!contract.comboLegs || contract.comboLegs->empty()) ENCODE_FIELD(0);
		else {
			typedef Contract::ComboLegList ComboLegList;
			const ComboLegList& comboLegs = *contract.comboLegs;
			ENCODE_FIELD((int)comboLegs.size());
...
void EClientSocketBase::reqScannerSubscription(int tickerId, const ScannerSubscription& subscription){
	// not conn ected?
	if (!m_connected) {
		m_pEWrapper->error(tickerId, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
		return;
...
void EClientSocketBase::reqFundamentalData(TickerId reqId, const Contract& contract, const IBString& reportType) {
	// not connected?
	if (!m_connected) {
...
void EClientSocketBase::placeOrder(OrderId id, const Contract &contract, const Order &order) {
	...
	if (m_serverVersion < MIN_SERVER_VER_DELTA_NEUTRAL_CONID) {
		if (order.deltaNeutralConId > 0 
			...
			m_pEWrapper->error(id, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support deltaNeutral parameters: ConId, SettlingFirm, ClearingAccount, ClearingIntent.");
			return;
		}
	}
	...
	if (m_serverVersion >= MIN_SERVER_VER_HEDGE_ORDERS) {
		...
		if (!IsEmpty(order.hedgeType)) ENCODE_FIELD(order.hedgeParam);
	}
	...
	if (m_serverVersion >= MIN_SERVER_VER_NOT_HELD) ENCODE_FIELD(order.notHeld);
...
void EClientSocketBase::reqExecutions(int reqId, const ExecutionFilter& filter) {
	...
	if (m_serverVersion >= MIN_SERVER_VER_EXECUTION_DATA_CHAIN) ENCODE_FIELD(reqId);
...
bool EClientSocketBase::checkMessages() {
	...
	if (bufferedRead() <= 0) return false;
...
int EClientSocketBase::processConnectAck(const char*& beginPtr, const char* endPtr) {
	...
	try {
		...
		if (m_serverVersion >= 20) DECODE_FIELD(m_TwsTime);
	...
	try {
		...
		switch(msgId) {
			...
			case OPEN_ORDER:
			{
				...
				} else if (version >= 23) DECODE_FIELD(order.exemptCode);
				...
				if (version >= 24) {
					...
					if (!IsEmpty(order.hedgeType)) DECODE_FIELD(order.hedgeParam);
				}

				if (version >= 25) DECODE_FIELD(order.optOutSmartRouting);

				DECODE_FIELD(order.clearingAccount); // ver 19 field
				DECODE_FIELD(order.clearingIntent); // ver 19 field

				if (version >= 22) DECODE_FIELD(order.notHeld);
			...
			case PORTFOLIO_VALUE:
			{
				...
				if (version == 6 && serverVersion() == 39) DECODE_FIELD(contract.primaryExchange);
			...
			case CONTRACT_DATA:
			{
				...
				int reqId = -1;
				if (version >= 3) DECODE_FIELD(reqId);
				...
				if (version >= 4) DECODE_FIELD(contract.underConId);
				if (version >= 5) {
			...
			case BOND_CONTRACT_DATA:
			{
				...
				if (version >= 4) DECODE_FIELD(contract.longName);
			...
			case EXECUTION_DATA:
			{
				...
				int reqId = -1;
				if (version >= 7) DECODE_FIELD(reqId);
				...
				if (version >= 8) DECODE_FIELD(exec.orderRef);
		...
		m_pEWrapper->error(NO_VALID_ID, SOCKET_EXCEPTION.code(), SOCKET_EXCEPTION.msg() + errMsg(e));
	}
#endif
