...
void EPosixClientSocket::eDisconnect() {
	if (m_fd >= 0 ) SocketClose(m_fd); // close socket
	m_fd = -1;
	// uninitialize Winsock DLL (only for Windows)
	SocketsDestroy();
	eDisconnectBase();
...
int EPosixClientSocket::send(const char* buf, size_t sz) {
	...
	if (nResult == -1 && !handleSocketError()) return -1;
...
bool EPosixClientSocket::handleSocketError() {
	...
	if (errno == ECONNREFUSED) getWrapper()->error(NO_VALID_ID, CONNECT_FAIL.code(), CONNECT_FAIL.msg());
	else getWrapper()->error(NO_VALID_ID, SOCKET_EXCEPTION.code(), SOCKET_EXCEPTION.msg() + strerror(errno));
	// reset errno
	errno = 0;
	eDisconnect();
	return false;
