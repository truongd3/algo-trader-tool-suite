...
struct ComboLeg {
   ...
   bool operator==( const ComboLeg &other) const {
      return (conId == other.conId && ratio == other.ratio &&
		        openClose == other.openClose && shortSaleSlot == other.shortSaleSlot &&
...
struct Contract {
   Contract() : conId(0), strike(0), includeExpired(false), comboLegs(NULL), underComp(NULL) {}
...
struct ContractDetails {
   ContractDetails() : minTick(0), priceMagnifier(0), underConId(0), callable(false)
	                   , putable(false), coupon(0), convertible(false), nextOptionPartial(false)	{}
...
inline void Contract::CloneComboLegs(ComboLegList& dst, const ComboLegList& src) {
...
inline void Contract::CleanupComboLegs(ComboLegList& legs) {
	if (legs.empty()) return;

	ComboLegList::iterator iter = legs.begin();
	const ComboLegList::iterator iterEnd = legs.end();

	for (; iter != iterEnd; ++iter) delete *iter;
