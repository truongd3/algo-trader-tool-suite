...
///////////////////////////////////////////////////////////
// member funcs
UnitTest::UnitTest() : m_pClient(new EPosixClientSocket(this)) , m_state(ST_CONNECT), m_sleepDeadline(0), m_buySignal(0) {}
...
bool UnitTest::connect(const char *host, unsigned int port, int clientId) {
	...
	if (bRes) cout << "Connected to %s:%d clientId:%d\n" << (!(host && *host) ? "127.0.0.1" : host) << port << clientId << endl;
	else cout << "Cannot connect to %s:%d clientId:%d\n" << (!(host && *host) ? "127.0.0.1" : host) << port << clientId << endl;
...
void UnitTest::processMessages() {
	...
	if (m_pClient->fd() >= 0) {
		...
		if (!m_pClient->isOutBufferEmpty()) FD_SET(m_pClient->fd(), &writeSet);
...
void UnitTest::orderStatus(OrderId orderId, const IBString &status, int filled,
	...
	if (orderId == m_pendingOrder) {
		if (m_state == ST_PLACEORDER_ACK && status == "Filled") m_state = ST_PLACETAIL;
	}
...
void UnitTest::updatePortfolio(const Contract& contract, int position, double marketPrice, 
							   double marketValue, double averageCost, double unrealizedPNL, double realizedPNL, const IBString& accountName){}
...
void UnitTest::updateMktDepth(TickerId id, int position, int operation, int side, double price, int size) {}
void UnitTest::updateMktDepthL2(TickerId id, int position, IBString marketMaker, int operation, int side, double price, int size) {}
...
void UnitTest::scannerData(int reqId, int rank, const ContractDetails &contractDetails, const IBString &distance,
